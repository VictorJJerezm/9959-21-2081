...
int main(){
  ...
  while(Dumbledore.GetHp() > 0 && Gandalf.GetHp() > 0){
    if(round % 2){
      ...
      current_wizzard = &Dumbledore; //current_wizzard es igual a la dirección de dumbeldore?
      current_wizzard->GetDamage(Gandalf.Damage());// ¿que es ->?
    }else{
      Harry.Speak("Gandalf Recibe Dano!!");
      current_wizzard = &Gandalf; //current_wizzard es igual a la dirección de gandalf
